...
# --- Config / options ---
CLASS_OPTIONS = ["1-7 CBSE","8th CBSE", "9th CBSE", "10th CBSE"]
SUBJECT_OPTIONS = ["Mathematics", "SST", "Science", "English", "Others"]
FACULTY_OPTIONS = ["Pravin K", "Shubham K", "Sujata G", "Shubham S","Shewta D"]
...
if submit:
    # Build pending attendance payload and ask for explicit confirmation before saving
    ...
    # store pending data in session_state so it survives re-runs
...
# If there's a pending attendance, show details and Confirm/Cancel buttons
if st.session_state.get("pending_attendance"):
    p = st.session_state["pending_attendance"]
    st.info("Please confirm the attendance entry below before saving:")
    st.table([p["header"], p["new_row"]])

    colc, colx = st.columns([1, 1])
    with colc:
        if st.button("Confirm Save", key="confirm_save"):
            # perform the same save logic as before using pending data
            date_str = p["date_str"]
            time_slot = p["time_slot"]
            new_row = p["new_row"]
            new_tuple = p["new_tuple"]
            cls = p["cls"]
            subj = p["subj"]
            faculty = p["faculty"]
            header = p["header"]
            ...
            try:
                ...
                if existing_text:
                    ...
                    if new_tuple in normalized_existing:
                        ...
                        # overlap check same date & same class (ignore subject)
                        conflict = False
                        try:
    ...
    with colx:
        if st.button("Cancel", key="cancel_save"):
            del st.session_state["pending_attendance"]
            st.info("Attendance entry cancelled.")
